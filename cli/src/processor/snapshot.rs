use sablier_network_program::state::{Registry, Snapshot, SnapshotEntry};
use solana_sdk::pubkey::Pubkey;

use crate::{client::Client, errors::CliError};

pub fn get(client: &Client, entry_id: Option<u64>) -> Result<(), CliError> {
    let registry_pubkey = sablier_client::network::objects::Registry::pubkey();
    let registry = client
        .get::<Registry>(&registry_pubkey)
        .map_err(|_err| CliError::AccountDataNotParsable(registry_pubkey.to_string()))?;

    let snapshot_pubkey =
        sablier_client::network::objects::Snapshot::pubkey(registry.snapshot_count - 1);
    let snapshot = client
...
pub fn get_snapshot_entry(
    ...
    let entry_pubkey =
        sablier_client::network::objects::SnapshotEntry::pubkey(snapshot_pubkey, entry_id);
